...
- MultiCh (Multiple Channels) - Any number of channels (A,B,...) may be
used, with a minimum of two.
...
%}
function [PL, PLI, ES] = P_TrainInterval_MultiCh_FixInterval(saveLocation, nameFileLib, varargin)
%% FILL IN THESE VALUES

%-------User Defined Variables-------%
Channels = [100,200,300,400,500]; % [A,B,...]
...
TrainingInterval = 20 * 20; %Interval between A, B, ... during training.

RepsPerCh_test = 5; % How many times the the stim pulse will be repeated
                 % on each channel during testing. (A A A ... B B B ...)
                 
RepsPerCh_train = 5; % How many times the stim pulse will be repeated on
...
NumPulses_train = length(Channels) * RepsPerCh_train; %total training pulses
...
for i=1:RepsPerCh_train %training
    for j=1:length(Channels)
        PulseTimes(length(Channels)*(i-1)+j+NumPulses_test) = Time;
        Time = Time + TrainingInterval;
    end
