...
- SimCh (Simultaneous Channels): Tetanization is performed for each channel
at the same times.
...
InterPulseDelay = 20 * 1000 * 5; % Time between trains of pulses.
...
RepsPerCh = 10; % How many times the tetanic pulse will be repeated on
...
NumPulses = length(Channels) * Trains * RepsPerCh; %total pulses
TotalDuration = FirstPulseDelay + Trains * InterPulseDelay;
PulseTimes = zeros(1,NumPulses); %initialize
PulseTimes(1) = FirstPulseDelay;
pulsecount = 1; %initialize

for j=1:Trains
    firsttime = PulseTimes(pulsecount);
    for i=1:length(Channels)
        PulseTimes(pulsecount:(pulsecount+RepsPerCh-1)) = (0:TetanicInterval:(TetanicInterval*RepsPerCh-1)) + firsttime;
        pulsecount = pulsecount + RepsPerCh;
    end
    PulseTimes(pulsecount) = firsttime + InterPulseDelay;
end
