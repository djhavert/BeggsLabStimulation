%{
[PL, PLI, ES] = P_TTX_Subtraction(saveLocation, nameFileLib, varargin)
...
%}
function [PL, PLI, ES] = P_TTX_Subtraction(saveLocation, nameFileLib, varargin)
...
%Channels{6,1} = [];
%......
% First element of 'Channels' cell array gives the channels that will be
% stimulated individually.
% Second element gives channels that will be stimulated in pairs
% Third element gives channels that will be stimulated in triplets
% etc...
...
AmplitudesDAC = [20];
%AmplitudesDAC = [5:5:40]; % MAX 42. The actual current amplitude of the largest
                    % peak in the pulse is (3*x/128)*CurrentRangeInAmps

InterPulseDelay = 20 * 20; % Time between stimulation of one channel 
                             % and the next.
                             % Note: All times are given in number of
                             % samples. Since the sample rate is 20000 HZ,
                             % one time unit here is equivalent to 50 us.

RepsPerCh = 100; % How many times the the stim pulse will be repeated on 
                 % each channel.
...
FirstPulseDelay = 5*20000+100; % Delay before the first pulse
...
for ii = 1:size(Channels,1)
  NumCombos = NumCombos + size(Channels{ii,1},1);
end
NumPulses = NumCombos * RepsPerCh * length(AmplitudesDAC);
TotalDuration = FirstPulseDelay + (NumPulses-1) * InterPulseDelay;
PulseTimes = (FirstPulseDelay:InterPulseDelay:TotalDuration);
...
PulseCount = 0;
EsIndex = 1;
for hh = 1:length(AmplitudesDAC)
  for ii = 1:size(Channels,1)
    for jj = 1:size(Channels{ii,1},1)
      for kk = 1:RepsPerCh
        PulseCount = PulseCount + 1;
        if PulseCount == 600
          disp('here');
        end
        for ll = 1:size(Channels{ii,1},2)
          EsIndex = EsIndex + 1;
          ES(EsIndex, 1) = PulseTimes(PulseCount);
          ES(EsIndex, 2) = RecOffOnOthers * Channels{ii,1}(jj,ll); 
          ES(EsIndex, 3) = hh; % real pulse
        end
      end
    end
  end
...
for ii = 1:length(ZeroAmpPulseTimes)
  ...
  ES(EsIndex, 3) = length(PLI); % zero amp pulse
end
