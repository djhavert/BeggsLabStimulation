%{
[PL, PLI, ES] = P_SeqCh_MultiAmp_FixFreq(saveLocation, nameFileLib, varargin)
...
- MultiAmp (Multiple Amplitudes) - Several stimulation amplitudes are
chosen and will be cycled through.
...
%}
function [PL, PLI, ES] = P_SeqCh_MultiAmp_FixFreq(saveLocation, nameFileLib, varargin)
%% FILL IN THESE VALUES

%-------User Defined Variables-------%
Channels = [331];
          % Each channel will be stimulated suequentailly (in the order 
...
NumPulses = length(Channels) * RepsPerCh * length(AmplitudesDAC);
...
for ii = 1:length(ZeroAmpPulseTimes)
  ...
  ES(PulseCount, 3) = length(PLI); % zero amp pulse
end
