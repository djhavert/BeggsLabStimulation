%{
[PL, PLI, ES] = P_RandCh_FixAmp_RandFreq(saveLocation, nameFileLib, varargin)
...
- RandCh (Random Channel) - The order in which the channels are
stimulated is random. However, the user can provide a blacklist of channels
to not choose from or a whitelist of channels to only choose from.
- FixAmp (Fixed Amplitude) - The stimulation amplitude is fixed and does
not change throughout the pattern.
- RandFreq (Random Frequency) - The stimulation times are chosen from a 
Poisson distribution. The user must provide an average stimulation rate,
lamda, in Hz, and a minimum threshold for time between stimulation events.
A fixed non-random frequency can also be selected
...
%}
function [PL, PLI, ES] = P_RandCh_FixAmp_RandFreq(saveLocation, nameFileLib, varargin)
%% FILL IN THESE VALUES

%-------User Defined Variables-------%
    % By default, all 512 channels can be selected for stimulation.
ChannelsBlacklist = [];
    % Blacklist channels will not be included in the list of selectable
    % channels for stimulation.
ChannelsWhitelist = [365:440,];
    % If a whitelist is given, only the channels given in the whitelist
    % will be selectable for stimulation. This will override any blacklist
    % given.
...
AmplitudesDAC = 10; % MAX 42. The actual current amplitude of the largest
                    % peak in the pulse is (3*x/128)*CurrentRangeInAmps

isPoisson = true;  % Poisson distrubted pulses? 
                   % If not, pulse rate will be constant
                    
PulseRate = 0.05 / 20000; % Hz / SampleRate -> Pulses per sample
                   % If Poisson, average pulse rate
                   % else, constant pulse rate.

TotalDuration = 90 * 60 * 20000;
                   % __ m * 60 s/m * 20000 samples/s          
                   % Length of time to stimulate.
                   % Note: All times are given in number of
                   % samples. Since the sample rate is 20000 HZ,
                   % one time unit here is equivalent to 50 us.

PoissonLowerLimit = 20 * 20; 
                   % 20 ms * 20 samples/ms
                   % Think of this as a refractory time between stimulation
                   % pulses. Two consecutive pulses are not allowed to
                   % occur within this time limit.
...
if isPoisson
  PulseTimes = FirstPulseDelay + PoissonTimes(PulseRate,TotalDuration,PoissonLowerLimit);
else
  PulseTimes = FirstPulseDelay + round(0:1/PulseRate:TotalDuration);
end
...
if ZeroAmpPulseTimes(1) < TimeBinWindow
  ZeroAmpPulseTimes(1) = ZeroAmpPulseTimes(1) + CommandPulseDelay;
end

% Create a list of selectable channels to choose randomly from
if isempty(ChannelsWhitelist)
  Channels = setdiff(1:512, ChannelsBlacklist);
else
  Channels = ChannelsWhitelist;
end
...
PulseCount = 1;
for ii = 1:length(PulseTimes)
  PulseCount = PulseCount + 1;
  ES(PulseCount, 1) = PulseTimes(PulseCount-1);
  ES(PulseCount, 2) = RecOffOnOthers * Channels(randi(length(Channels))); 
...
actual_lambda = 1/(mean(diff(ES(ES(:,2)>1,1)))/20000);
disp(actual_lambda);
disp(["Actual mean fire rate is ", actual_lambda, " Hz"]);
