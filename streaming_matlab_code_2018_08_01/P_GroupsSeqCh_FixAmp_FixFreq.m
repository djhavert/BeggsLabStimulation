%{
[PL, PLI, ES] = P_GroupsSeqCh_FixAmp_FixFreq(saveLocation, nameFileLib, varargin)
...
- GroupsSeqCh (Groups Sequential Channels) - The order in which the channels are
stimulated is sequential and cyclical and groups of multiple sets are stimulated 
simultaneously in order given. As an example, 2 groups of 2 sets each:
{
[  1,  2,  3], [ 11, 12, 13, 14];
[101,102,103,104], [111,112,113];
}
then the order in which the channels are stimulated is 
((1,2,3) (11,12,13,14) repeated), then (101,102,103,104) (111,112,1113) repeated) 
...
%}
function [PL, PLI, ES] = P_GroupsSeqCh_FixAmp_FixFreq(saveLocation, nameFileLib, varargin)
...
Channels = {
          ...
          % Each array in a row represents a set of channels that will be
          % stimulated simultaneously
          % All of the sets in a row form a group
          % Each set in a group will be presented one at a time, simultaneously,
          % or with a small delay between each with the option to stimulate
          % in different orderings.
          % After all combinations of sets in a group are presented and
          % repeated the given number of times, it moves on to the next
          % group. 
          % RIGHT NOW THE CODE ONLY FUNCTIONS IF THERE IS A SINGLE GROUP

BinaryCounting = false; % Determines if the group of sets should be 
                       % broken up into 2^n smaller subgroups of all possible
                       % combinations of firing. Example: A group of 3 has
                       % 2^3=8 possible combinations of firing those three
                       % sets (000 001 010 011 100 101 110 111).
...
GroupPulseDelay = 50 * 20; % Small delay between stimulating each CH in
                              % a grouping.

RepsPerSet = 500; % How many times the the stim pulse will be repeated for 
...
for ii = 1:size(Channels,1) %for each set...
  for jj = 1:RepsPerSet  %for each repition of the set...
      ...
      if sum(BinaryCounts(kk,:)) == 0 %if they are all zero
        ...
        for ll = 1:size(Channels,2) %for each channel in the set...
          ...
          if count %if channel  is nonzero
            for mm = 1:size(Channels{ii,ll},2)
              EsIndex = EsIndex + 1;
              ES(EsIndex, 1) = PulseTimes(PulseCount) + (count-1)*GroupPulseDelay; %
              ES(EsIndex, 2) = RecOffOnOthers * Channels{ii,ll}(mm); 
